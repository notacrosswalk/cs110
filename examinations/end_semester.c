...
void inputParagraph()
{
    ...
    fflush(stdin);
    scanf("%[^\n]s", paragraph); // Might need a '\n' at the beginning if fflush(stdin) does not work
}
...
{
    for(int i = 0; i < SIZE_WORD; i++) tempWord[i] = '\0';
    char *p = begin;
    int j = 0;
    while(p <= end && j < SIZE_WORD)
    {
        tempWord[j] = *p;
        j++;
        p++;
